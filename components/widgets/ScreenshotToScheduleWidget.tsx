    ...
    <CardContent className="h-full">
      <div className="flex items-start justify-between gap-3">
        <div>
          <div className="flex items-center gap-2">
            <Upload className="h-4 w-4 text-black/80 dark:text-white/85" />
            <div className="text-sm font-semibold">Screenshot → Schedule</div>
          </div>
          <div className="mt-1 text-xs text-black/80 dark:text-white/85">Upload a screenshot (GameChanger etc.) and convert it into an event.</div>
        </div>
      </div>
      ...
      <div className="mt-3 rounded-2xl border border-black/20 bg-white/95 p-3 dark:bg-black/55">
        <div className="flex flex-col gap-2">
          ...
          {previewUrl ? (
            <div className="overflow-hidden rounded-2xl border border-black/20 bg-white/95">
              {/* eslint-disable-next-line @next/next/no-img-element */}
              <img src={previewUrl} alt="Screenshot preview" className="max-h-[240px] w-full object-contain" />
            </div>
          ) : (
            <div className="text-sm text-black/80 dark:text-white/85">Choose an image to preview.</div>
          )}

          <div className="flex items-center justify-between gap-3">
            <div className="text-xs text-black/80 dark:text-white/85">{file ? `Ready: ${file.name}` : "No file selected"}</div>
            <Button
          ...
          <>
            ...
            <div className="space-y-3">
              ...
              <div className="grid grid-cols-1 gap-2 sm:grid-cols-3">
                <div>
                  <div className="text-xs font-medium text-black/80 dark:text-white/85">Date</div>
                  <Input type="date" value={editedDate} onChange={(e) => setEditedDate(e.target.value)} className="mt-1" />
                </div>
                <div>
                  <div className="text-xs font-medium text-black/80 dark:text-white/85">Time</div>
                  <Input type="time" value={editedTime} onChange={(e) => setEditedTime(e.target.value)} className="mt-1" />
                </div>
                <div>
                  <div className="text-xs font-medium text-black/80 dark:text-white/85">Team</div>
                  <Input value={editedTeam} onChange={(e) => setEditedTeam(e.target.value)} placeholder="e.g. Warriors" className="mt-1" />
                </div>
              </div>

              <div>
                <div className="text-xs font-medium text-black/80 dark:text-white/85">Assign to family member</div>
                <select
                  className="h-10 mt-1 w-full rounded-2xl border border-black/20 bg-white/95 px-3 text-sm dark:border-white/20 dark:bg-black/55"
              ...
              <div>
                <div className="text-xs font-medium text-black/80 dark:text-white/85">Notes (optional)</div>
