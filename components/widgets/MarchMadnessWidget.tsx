  ...
  return (
    <CardContent className="h-full">
      <div className="flex items-start justify-between gap-3">
        <div>
          <div className="flex items-center gap-2">
            <Trophy className="h-4 w-4 text-black/80 dark:text-white/85" />
      ...
      <div className="grid grid-cols-2 gap-2">
        <div className="rounded-2xl border border-black/20 bg-white/95 p-2 dark:bg-black/55">
          <div className="flex items-center justify-between gap-2">
            <div className="flex items-center gap-2">
              <Clock className="h-4 w-4 text-black/80 dark:text-white/85" />
              <div className="text-xs font-semibold text-black/80 dark:text-white/85">Date</div>
            </div>
            <div className="text-xs text-black/80 dark:text-white/85">{selectedDateIso}</div>
          </div>
          <div className="mt-2">
            <Input
              type="date"
        ...
        <div className="rounded-2xl border border-black/20 bg-white/95 p-2 dark:bg-black/55">
          <div className="text-xs font-semibold text-black/80 dark:text-white/85">Filter</div>
      ...
      <div className="space-y-2 overflow-auto pr-1" style={{ maxHeight: 240 }}>
        {isLoading ? (
          <div className="text-sm text-black/80 dark:text-white/85">Loading…</div>
        ) : top.length === 0 ? (
          <div className="text-sm text-black/80 dark:text-white/85">No games for this filter.</div>
        ) : (
          top.map((g) => {
            ...
            return (
              <button
                key={g.id}
                className="w-full rounded-2xl border border-black/20 bg-white/95 p-3 text-left hover:bg-white dark:bg-black/45 dark:hover:bg-black/30"
