    ...
    <CardContent className="h-full">
      <div className="flex items-start justify-between gap-3">
        <div>
          ...
          <div className="mt-1 text-xs text-black/80 dark:text-white/85">Tap a tile for instant access.</div>
        </div>
      ...
      <div className="grid grid-cols-2 gap-2">
        {quickLinks.map((l) => {
          ...
          return (
            <div key={l.id} className="relative overflow-hidden rounded-2xl border border-black/20 bg-white/95 p-3 dark:bg-black/45">
              <button
              ...
              >
                <div className="flex items-center justify-between gap-3 w-full">
                  <div className="flex items-center gap-2 min-w-0">
                    <Icon className="h-4 w-4 text-black/80 dark:text-white/85" />
                ...
                <div className="mt-auto text-xs text-black/80 dark:text-white/85 line-clamp-2">
                  {l.url}
                </div>
              </button>
              ...
              <div className="absolute right-2 top-2 flex gap-1">
                <button
                  className="rounded-full bg-white/95 p-1 text-black/80 hover:bg-white"
                  onClick={(e) => {
                    e.preventDefault();
                    e.stopPropagation();
                    startEdit(l);
        ...
        {quickLinks.length === 0 ? <div className="col-span-2 text-sm text-black/80 dark:text-white/85">No quick links yet.</div> : null}
      </div>
      ...
      <Dialog open={dialogOpen} onOpenChange={setDialogOpen}>
        <DialogContent className="max-w-xl">
          ...
          <div className="mt-3 grid grid-cols-1 gap-3">
              <div>
                <div className="text-xs font-medium text-black/80 dark:text-white/85">Label</div>
              <Input value={label} onChange={(e) => setLabel(e.target.value)} className="mt-1" placeholder="e.g. GameChanger" />
            </div>
            <div>
                <div className="text-xs font-medium text-black/80 dark:text-white/85">URL</div>
              <Input value={url} onChange={(e) => setUrl(e.target.value)} className="mt-1" placeholder="https://..." />
            </div>
            <div className="grid grid-cols-1 gap-2 sm:grid-cols-2">
              <div>
                <div className="text-xs font-medium text-black/80 dark:text-white/85">Icon name</div>
                <Input value={icon} onChange={(e) => setIcon(e.target.value)} className="mt-1" placeholder="Camera, Ticket, Cloud…" />
              </div>
              <div>
                <div className="text-xs font-medium text-black/80 dark:text-white/85">Tile color</div>
                <div className="mt-1 flex items-center gap-2">
                  <input
                    type="color"
                    value={color}
                    onChange={(e) => setColor(e.target.value)}
                    className="h-10 w-16 rounded-2xl border border-black/20 bg-white/95 p-1 dark:border-white/20 dark:bg-black/55"
                  />
                </div>
              </div>
            </div>
