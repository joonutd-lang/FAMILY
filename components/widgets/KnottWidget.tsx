...
export function KnottWidget() {
  ...
  const { data, isLoading, error } = useQuery({
    ...
    queryFn: () => knottsService.getHoursForDate(new Date()),
    staleTime: 30 * 60_000,
  });
  ...
  return (
    <CardContent className="h-full">
      <div className="flex items-start justify-between gap-3">
        <div>
          <div className="flex items-center gap-2">
            <CalendarClock className="h-4 w-4 text-black/60 dark:text-white/60" />
            <div className="text-sm font-semibold">Knott’s Berry Farm</div>
          </div>
          <div className="mt-1 text-xs text-black/60 dark:text-white/60">Hours for today (mock).</div>
        </div>
      ...
      <div className="rounded-2xl border border-black/10 bg-white/60 p-3 dark:bg-black/30">
        ...
        <div className="mt-2 text-sm text-black/70 dark:text-white/70">{data?.hoursText ?? "—"}</div>
      </div>
