    ...
    <CardContent className="h-full">
      <div className="flex items-start justify-between gap-3">
        <div className="min-w-0">
          <div className="flex items-center gap-2">
            <Cloud className="h-4 w-4 text-black/80 dark:text-white/85" />
            <div className="truncate text-sm font-semibold">Weather</div>
          </div>
          <div className="mt-1 text-xs text-black/80 dark:text-white/85 truncate">{DEFAULT_LOCATION.locationLabel}</div>
        </div>
      ...
      {error ? (
        <div className="text-sm text-red-600">Weather unavailable.</div>
          ...
          <div className="rounded-2xl border border-black/20 bg-white/95 p-3 dark:bg-black/55 h-full flex flex-col">
          <div className="flex items-start justify-between gap-3">
            <div>
              <div className="text-xs text-black/80 dark:text-white/85">Now</div>
              <div className="mt-1 text-4xl font-semibold tracking-tight">
                {isLoading || !wx ? "—" : `${Math.round(wx.temperatureC)}°C`}
              </div>
              <div className="mt-1 text-xs text-black/80 dark:text-white/85">{wx ? wx.description : ""}</div>
            </div>
            {wx ? (
          ...
          <div className="mt-3 grid grid-cols-2 gap-2">
            <div className="rounded-2xl border border-black/20 bg-white/95 p-2 dark:bg-black/45">
              <div className="flex items-center justify-between gap-2">
                <div className="flex items-center gap-2">
                  <Droplets className="h-3.5 w-3.5 text-black/80 dark:text-white/85" />
                  <div className="text-xs text-black/80 dark:text-white/85">Precip</div>
                </div>
            ...
            <div className="rounded-2xl border border-black/20 bg-white/95 p-2 dark:bg-black/45">
              <div className="flex items-center justify-between gap-2">
                <div className="flex items-center gap-2">
                  <Wind className="h-3.5 w-3.5 text-black/80 dark:text-white/85" />
                  <div className="text-xs text-black/80 dark:text-white/85">Wind</div>
                </div>
          ...
          <div className="mt-auto pt-3 text-xs text-black/80 dark:text-white/85">
