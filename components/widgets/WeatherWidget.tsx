  ...
  return (
    <CardContent className="h-full">
      <div className="flex items-start justify-between gap-3">
        <div className="min-w-0">
          <div className="flex items-center gap-2">
            <Cloud className="h-4 w-4 text-black/60 dark:text-white/60" />
            <div className="truncate text-sm font-semibold">Weather</div>
          </div>
          <div className="mt-1 text-xs text-black/60 dark:text-white/60 truncate">{DEFAULT_LOCATION.locationLabel}</div>
        </div>
      ...
      {error ? (
        ...
        <div className="rounded-2xl border border-black/10 bg-white/60 p-3 dark:bg-black/30 h-full flex flex-col">
          <div className="flex items-start justify-between gap-3">
            <div>
              <div className="text-xs text-black/60 dark:text-white/60">Now</div>
              <div className="mt-1 text-4xl font-semibold tracking-tight">
                {isLoading || !wx ? "—" : `${Math.round(wx.temperatureC)}°C`}
              </div>
              <div className="mt-1 text-xs text-black/60 dark:text-white/60">{wx ? wx.description : ""}</div>
            </div>
            {wx ? (
          ...
          <div className="mt-3 grid grid-cols-2 gap-2">
            <div className="rounded-2xl border border-black/10 bg-white/70 p-2 dark:bg-black/20">
              <div className="flex items-center justify-between gap-2">
                <div className="flex items-center gap-2">
                  <Droplets className="h-3.5 w-3.5 text-black/60 dark:text-white/60" />
                  <div className="text-xs text-black/60 dark:text-white/60">Precip</div>
                </div>
            ...
            <div className="rounded-2xl border border-black/10 bg-white/70 p-2 dark:bg-black/20">
              <div className="flex items-center justify-between gap-2">
                <div className="flex items-center gap-2">
                  <Wind className="h-3.5 w-3.5 text-black/60 dark:text-white/60" />
                  <div className="text-xs text-black/60 dark:text-white/60">Wind</div>
                </div>
          ...
          <div className="mt-auto pt-3 text-xs text-black/60 dark:text-white/60">
